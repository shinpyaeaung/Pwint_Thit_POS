ALTER TABLE app.sales ADD COLUMN request_id uuid UNIQUE;
ALTER TABLE app.sales ADD COLUMN request_payload jsonb;
ALTER TABLE app.sales ADD COLUMN invoice_document jsonb;
CREATE SEQUENCE app.invoice_sequence;
CREATE FUNCTION app.pos_sale(d jsonb, actor uuid, discounts boolean, below_cost boolean, preview boolean) RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE old app.sales; customer app.customers; product app.products; pack app.product_units; x jsonb; alloc jsonb; lines jsonb='[]'; allocations jsonb; stock record;
 ...
 doc jsonb; quote text; sale uuid; sale_item uuid; sale_batch uuid; payment uuid; invoice text; line_no int=0;
BEGIN
 IF NOT preview THEN
  ...
  IF FOUND THEN IF old.request_payload<>d THEN RAISE EXCEPTION 'pos: This checkout request was already used with different contents.' USING ERRCODE='23514'; END IF; RETURN jsonb_build_object('id',old.id,'invoice_number',old.invoice_number); END IF;
 END IF;
 ...
 FOR x IN SELECT value FROM jsonb_array_elements(d->'items') LOOP
  ...
  IF line_total<line_cost AND (NOT below_cost OR btrim(d->>'reason')='') THEN RAISE EXCEPTION 'pos: Sale is below actual batch cost. Approval permission and a reason are required.' USING ERRCODE='23514'; END IF;
 ...
 quote=md5(doc::text);
 IF preview THEN RETURN doc||jsonb_build_object('quote_hash',quote); END IF;
...
CREATE FUNCTION app.pos_prices(d jsonb,actor uuid) RETURNS void LANGUAGE plpgsql AS $$
DECLARE p app.products; before jsonb;
BEGIN
 SELECT * INTO p FROM app.products WHERE id=(d->>'product_id')::uuid FOR UPDATE;
 IF NOT FOUND OR p.version::text<>d->>'version' THEN RAISE EXCEPTION 'pos: Product changed. Reload prices.' USING ERRCODE='23514'; END IF;
 SELECT to_jsonb(u) INTO before FROM app.product_units u WHERE product_id=p.id AND unit_code=d->>'unit_code';
 IF NOT FOUND THEN RAISE EXCEPTION 'pos: Selling unit no longer exists.' USING ERRCODE='23514'; END IF;
 UPDATE app.product_units SET retail_price_mmk=(d->>'retail_price_mmk')::numeric,wholesale_price_mmk=(d->>'wholesale_price_mmk')::numeric WHERE product_id=p.id AND unit_code=d->>'unit_code';
 UPDATE app.products SET version=version+1 WHERE id=p.id;
 INSERT INTO app.audit_logs(actor_id,action,entity_type,entity_id,old_value,new_value) VALUES(actor,'products.prices','products',p.id,before,d);
END $$;
CREATE FUNCTION app.pos_public_document(doc jsonb, costs boolean, profits boolean) RETURNS jsonb LANGUAGE sql IMMUTABLE AS $$
 SELECT (doc-CASE WHEN costs THEN ARRAY[]::text[] ELSE ARRAY['cost_mmk'] END-CASE WHEN profits THEN ARRAY[]::text[] ELSE ARRAY['profit_mmk'] END)
 ||jsonb_build_object('lines',coalesce((SELECT jsonb_agg(value-CASE WHEN costs THEN ARRAY[]::text[] ELSE ARRAY['cost_mmk','allocations'] END) FROM jsonb_array_elements(doc->'lines')),'[]'::jsonb))
$$;
