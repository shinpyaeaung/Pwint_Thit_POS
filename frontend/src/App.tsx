...
export default function App() {
  ...
  const page = path === '/customers' || path.startsWith('/customers/') ? <Customers current={user} /> : path === '/pos' ? <POS current={user} /> : path === '/sales' || path.startsWith('/sales/') ? <Sales /> : path === '/batches' ? <Batches current={user} /> : path === '/receiving' || path.startsWith('/receiving/') ? <Receipts current={user} /> : path === '/inventory' || path.startsWith('/inventory/') ? <Inventory current={user} /> : path === '/shipments' || path.startsWith('/shipments/') ? <Shipments current={user} /> : path === '/purchases' || path.startsWith('/purchases/') ? <Purchases current={user} /> : path === '/purchasing-settings' ? <CurrencySettings current={user} /> : path === '/suppliers' || path.startsWith('/suppliers/') ? <Suppliers current={user} /> : path === '/products' || path.startsWith('/products/') ? <Products current={user} /> : path === '/catalog' ? <CatalogPage /> : path === '/users' ? <Users current={user} /> : path === '/permissions' ? <Permissions /> : path === '/design-system' ? <DesignSystem current={user} /> : <Workspace />
