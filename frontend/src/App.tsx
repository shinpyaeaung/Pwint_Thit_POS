...
import { protectedPages } from '@/permissions'
...
export default function App() {
  ...
  const page = path === '/users' ? <Users current={user} /> : path === '/permissions' ? <Permissions /> : path === '/design-system' ? <DesignSystem current={user} /> : <Workspace />
  return <AppShell user={user} onLogout={() => logout.mutate()} signingOut={logout.isPending}>
    ...
    {!known ? <EmptyState title="Page not found" action={<a href="/">Return to workspace</a>} /> : required ? <PermissionGuard user={user} permission={required} fallback={restricted}>{page}</PermissionGuard> : page}
  </AppShell>
}
