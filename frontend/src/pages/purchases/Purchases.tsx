...
function PurchaseDetails({ p, current }: { p: Purchase; current: CurrentUser }) {
 return <><PageHeader eyebrow="Purchasing / Purchase details" title={p.purchase_number} description={`${p.supplier_name} · ${new Date(p.purchased_at).toLocaleDateString()}`} actions={<Button asChild variant="outline"><a href="/purchases">All purchases</a></Button>} />
  ...
  {p.can_view_cost&&<div className="mt-6"><CostJourney sources={[{amount:p.total_original!,currency:p.currency_code,rate:p.mmk_per_unit!,reference:p.purchase_number}]} converted={p.total_mmk} canPreviewProfit={can(current,permissions.financeViewProfit)} scope={`${p.purchase_number} · complete purchase`} notes="Transport and landed costs are tracked separately for each shipment allocation below."/></div>}{can(current,permissions.financeViewLandedCost)&&<CostHistory kind="purchases" id={p.id} current={current}/>}<h2 className="mb-4 mt-6 text-sm font-semibold">Purchase items</h2><DataTable caption="Purchase items" rows={p.items || []} rowKey={i => i.id} columns={[
