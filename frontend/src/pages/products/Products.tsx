...
function ProductList({ catalog, current }: { catalog: Catalog; current: CurrentUser }) {
  ...
  return <><PageHeader eyebrow="Product & stock" title="Products" description="Your product catalog, packaging and stock thresholds." actions={<><PermissionGuard user={current} permission={permissions.catalogManage}><Button variant="outline" asChild><a href="/catalog">Manage catalog</a></Button></PermissionGuard><PermissionGuard user={current} permission={permissions.productsCreate}><Button asChild><a href="/products/new"><Plus />Add Product</a></Button></PermissionGuard></>} />
    <FilterBar onReset={search || category || brand || status !== 'all' ? reset : undefined} summary={query.data ? `${query.data.total} products` : undefined}><SearchInput label="Search products" placeholder="Name, SKU or barcode…" value={search} onValueChange={v => { setSearch(v); setPage(1) }} /><label className="flex items-center gap-2 text-xs">Status<select aria-label="Status" className="field w-auto" value={status} onChange={e => { setStatus(e.target.value); setPage(1) }}><option value="all">All current</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></label>{(['categories', 'brands'] as const).map(kind => <label key={kind} className="flex items-center gap-2 text-xs">{kind === 'categories' ? 'Category' : 'Brand'}<select aria-label={kind === 'categories' ? 'Category' : 'Brand'} className="field max-w-44" value={kind === 'categories' ? category : brand} onChange={e => { (kind === 'categories' ? setCategory : setBrand)(e.target.value); setPage(1) }}><option value="">All</option>{catalog[kind].map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></label>)}</FilterBar>
...
function ProductDetails({ product: p, catalog, current }: { product: Product; catalog: Catalog; current: CurrentUser }) {
  ...
  return <><PageHeader eyebrow="Product & stock / Product details" title={p.name} description={`SKU · ${p.sku}`} actions={<><Button variant="outline" asChild><a href="/products">All products</a></Button>{!p.archived_at && <><PermissionGuard user={current} permission={permissions.productsUpdate}><Button asChild><a href={`/products/${p.id}/edit`}><Pencil />Edit Product</a></Button></PermissionGuard><PermissionGuard user={current} permission={permissions.productsDelete}><Button variant="outline" onClick={() => setConfirm(true)}><Archive />Archive product</Button></PermissionGuard></>}</>} /><div className="mb-5"><ProductStatus product={p} /></div>
    ...
    <p className="mt-5 text-xs text-muted-foreground">Created {new Date(p.created_at).toLocaleString()} · Updated {new Date(p.updated_at).toLocaleString()}</p><ConfirmDialog open={confirm} onOpenChange={setConfirm} title="Archive this product?" description={`${p.name} will be removed from the current catalog. Its SKU, barcodes and history remain reserved.`} confirmLabel="Archive product" destructive onConfirm={() => archive.mutate()} busy={archive.isPending} error={archive.error?.message} />
  </>
}
