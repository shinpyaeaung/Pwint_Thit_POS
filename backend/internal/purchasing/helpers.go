...
func dbError(c *gin.Context, err error) {
	...
	if errors.As(err, &pe) {
		switch pe.Code {
		case "23505":
			fail(c, 409, "That purchase number, supplier invoice, currency or rate date already exists.")
			return
		case "23503", "23514", "22003", "22P02":
			fail(c, 400, "Invalid purchasing data.")
			return
		}
	}
	fail(c, 503, "Purchasing data is unavailable. Please try again.")
}
func pathID(c *gin.Context) (pgtype.UUID, bool) {
	id, e := uuid(c.Param("id"))
	if e != nil || !id.Valid {
		fail(c, 400, "Invalid purchasing ID.")
		return id, false
	}
	return id, true
}
