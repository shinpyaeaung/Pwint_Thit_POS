...
func dbError(c *gin.Context, err error) {
	...
	if errors.As(err, &pe) {
		if pe.Code == "23514" && strings.HasPrefix(pe.Message, "receiving: ") {
			fail(c, 409, strings.TrimPrefix(pe.Message, "receiving: "))
			return
		}
		switch pe.Code {
		case "23505":
			fail(c, 409, "That receipt or adjustment request already exists. Reload before retrying.")
			return
		case "23503", "23514", "22003", "22P02":
			fail(c, 400, "Invalid receiving or inventory data.")
			return
		}
	}
	fail(c, 503, "Receiving or inventory data is unavailable. Please try again.")
}
func pathID(c *gin.Context) (pgtype.UUID, bool) {
	id, e := uuid(c.Param("id"))
	if e != nil || !id.Valid {
		fail(c, 400, "Invalid shipment ID.")
		return id, false
	}
	return id, true
}
