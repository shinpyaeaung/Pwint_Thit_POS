...
func dbError(c *gin.Context, err error) {
	...
	if errors.As(err, &pe) {
		switch pe.Code {
		case "23505":
			fail(c, 409, "That shipment number, warehouse code or request already exists. Refresh the shipment before retrying.")
			return
		case "23503", "23514", "22003", "22P02":
			fail(c, 400, "Invalid shipment data.")
			return
		}
	}
	fail(c, 503, "Shipment data is unavailable. Please try again.")
}
