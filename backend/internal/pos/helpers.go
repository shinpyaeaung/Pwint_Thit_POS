...
func dbError(c *gin.Context, err error) {
	...
	if errors.As(err, &pe) {
		if pe.Code == "23514" && strings.HasPrefix(pe.Message, "pos: ") {
			fail(c, 409, strings.TrimPrefix(pe.Message, "pos: "))
			return
		}
		switch pe.Code {
		case "23505":
			fail(c, 409, "That checkout request already exists. Reload before retrying.")
			return
		case "23503", "23514", "22003", "22P02":
			fail(c, 400, "Invalid sale or payment data.")
			return
		}
	}
	fail(c, 503, "Sales data is unavailable. Please try again.")
}
